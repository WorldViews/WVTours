...
*/
WV.CoordinateSystem = class {

    constructor(lat, lon, alt, heading, sx, sy) {
        sx = sx || 1;
        sy = sy || 1;
    ...
    update(lat, lon, alt, heading, sx, sy) {
        var cs = this;
        cs.lat = lat;
        cs.lon = lon;
        if (sx)
    ...
    xyzToLla(xyz) {
        ...
        var lxyz = M33.transform(this.modelToLocal, xyz);
        var gxyz = V3.add(this.origin_xyz, M33.transform(this.localToGlobal, lxyz));
